#!/usr/bin/env python3
"""
Comprehensive Attention Norm Test for Mistral24B Model
...
Tests the attention_norm layer: PixtralRMSNorm((1024,), eps=1e-05) within PixtralAttentionLayer
This is the RMSNorm that precedes attention in each transformer layer.
"""
...
@dataclass
class ComprehensiveAttentionNormTestResult:
...
class InputCaptureHook:
    """Hook to capture inputs to the attention_norm RMSNorm layers across all 24 transformer layers."""
    ...
    def __call__(self, module, input, output):
        ...
        if len(input) > 0:
            ...
            logger.info(
                f"Captured attention_norm input tensor from layer {self.layer_idx} with shape: {captured_input.shape}"
            )
...
class MultiLayerInputCaptureHook:
    """Hook system to capture inputs from all 24 attention_norm layers."""
    ...
    def attach_to_all_layers(self, reference_model):
        """Attach hooks to all transformer layer attention_norm modules."""
        for layer_idx in range(self.num_layers):
            try:
                # Access each layer's attention_norm
                attention_norm_module = reference_model.vision_tower.transformer.layers[layer_idx].attention_norm

                # Create hook for this layer
                layer_hook = InputCaptureHook(layer_idx=layer_idx)
                layer_hook.attach_to_module(attention_norm_module)

                self.layer_hooks.append(layer_hook)
                logger.info(f"Attached hook to attention_norm in layer {layer_idx}")
...
def capture_real_attention_norm_inputs(model_args) -> Dict[int, torch.Tensor]:
    """
    Capture real input tensors to all 24 attention_norm RMSNorm layers by running the full multimodal model
    ...
    Returns:
        Dict[int, torch.Tensor]: Dictionary mapping layer index to input tensor for each attention_norm layer
    """
    logger.info("Capturing real attention_norm inputs from all 24 layers in full multimodal model...")
    ...
    try:
        with torch.no_grad():
            ...
            try:
                # Just run a forward pass to capture the attention_norm input
...
def run_ttnn_attention_norm(mesh_device, device_name: str, model_args, state_dict, input_tensor, layer_idx=0):
    """Run TTNN RMSNorm implementation for a specific mesh device and layer."""
    logger.info(f"Running TTNN attention_norm layer {layer_idx} on {device_name} device...")

    # RMSNorm parameters
    dim = model_args.vision_dim  # 1024
    eps = 1e-5  # Standard epsilon for vision RMSNorm

    # Use the specific transformer layer's attention_norm prefix
...
def comprehensive_attention_norm_test(
    mesh_devices: Dict[str, Any],
    pcc_threshold: float = 0.99,
    atol_threshold: float = 1e-2,
    rtol_threshold: float = 1e-2,
    save_results: bool = True,
    results_dir: str = "comprehensive_attention_norm_test_results",
) -> ComprehensiveAttentionNormTestResult:
    """
    ...
    Returns:
        ComprehensiveAttentionNormTestResult with all statistics and metrics
    """

    logger.info("=" * 80)
    logger.info("COMPREHENSIVE MISTRAL24B ATTENTION NORM TEST WITH MULTIMODAL REAL INPUTS")
    logger.info("=" * 80)
    ...
    test_config = {
        ...
        "layer": "attention_norm PixtralRMSNorm((1024,), eps=1e-05)",
    ...
    logger.info(f"attention_norm RMSNorm params: dim={dim}, eps={eps}")
    ...
    captured_inputs_by_layer = capture_real_attention_norm_inputs(model_args)
    ...
    for layer_idx in captured_inputs_by_layer.keys():
        # Get the attention_norm from this specific transformer layer
        attention_norm_ref = reference_model.vision_tower.transformer.layers[layer_idx].attention_norm

        # Load the appropriate weights for this layer
        attention_norm_prefix = f"vision_tower.transformer.layers.{layer_idx}.attention_norm"
        attention_norm_state_dict = {
            k[len(attention_norm_prefix + ".") :]: v
            for k, v in state_dict.items()
            if k.startswith(attention_norm_prefix + ".")
        }

        if attention_norm_state_dict:
            logger.info(
                f"Loading layer {layer_idx} attention_norm weights with keys: {list(attention_norm_state_dict.keys())}"
            )
            attention_norm_ref.load_state_dict(attention_norm_state_dict)
            attention_norm_ref.eval()

            # Run PyTorch reference for this layer
            input_tensor = captured_inputs_by_layer[layer_idx]
            with torch.no_grad():
                torch_output = attention_norm_ref(input_tensor)
    ...
    for layer_idx in captured_inputs_by_layer.keys():
        ...
        for device_name, mesh_device in mesh_devices.items():
            ...
            try:
                tt_output_torch = run_ttnn_attention_norm(
    ...
    result = ComprehensiveAttentionNormTestResult(
    ...
    if save_results:
        ...
        # Save JSON report
        json_path = results_path / "comprehensive_attention_norm_test_results.json"
...
def test_comprehensive_attention_norm_cross_mesh(mesh_device_configs):
    """Comprehensive attention_norm test across multiple mesh device configurations."""
    logger.info("Running comprehensive attention_norm test across mesh devices...")
    ...
    try:
        result = comprehensive_attention_norm_test(
            mesh_devices=available_devices, pcc_threshold=0.99, save_results=True
        )

        # Assert overall test passed
        assert result.test_passed, f"attention_norm test failed. Check detailed results in logs."
        ...
        for device_name, mesh_result in result.mesh_results.items():
            ...
            ), f"attention_norm test failed on {device_name} device for representative layer"
...
def test_single_device_attention_norm(mesh_device):
    """Test attention_norm on a single mesh device configuration."""
    logger.info("Running single device attention_norm test...")
    ...
    try:
        result = comprehensive_attention_norm_test(
            ...
            pcc_threshold=0.99,
            save_results=True,
            results_dir=f"attention_norm_test_results_{device_name.lower()}",
        )

        assert result.test_passed, f"attention_norm test failed on {device_name}"

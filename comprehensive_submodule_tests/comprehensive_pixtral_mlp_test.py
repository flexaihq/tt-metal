#!/usr/bin/env python3
"""
Comprehensive PixtralMLP Test for Mistral24B Model
...
Tests the feed_forward layer: PixtralMLP with gate_proj, up_proj, down_proj, and SiLU activation
This is the MLP that follows FFN normalization in each transformer layer.
"""
...
@dataclass
class ComprehensivePixtralMLPTestResult:
...
class InputCaptureHook:
    """Hook to capture inputs to the feed_forward PixtralMLP layers across all 24 transformer layers."""
    ...
    def __call__(self, module, input, output):
        ...
        # MLP forward signature: forward(self, x)
        if len(input) > 0:
            # Capture the input tensor
            captured_input = input[0].clone().detach()
            self.captured_inputs.append(captured_input)
            logger.info(
                f"Captured feed_forward MLP input tensor from layer {self.layer_idx} with shape: {captured_input.shape}"
            )
...
class MultiLayerInputCaptureHook:
    """Hook system to capture inputs from all 24 feed_forward MLP layers."""
    ...
    def attach_to_all_layers(self, reference_model):
        """Attach hooks to all transformer layer feed_forward modules."""
        for layer_idx in range(self.num_layers):
            try:
                # Access each layer's feed_forward MLP
                feed_forward_module = reference_model.vision_tower.transformer.layers[layer_idx].feed_forward

                # Create hook for this layer
                layer_hook = InputCaptureHook(layer_idx=layer_idx)
                layer_hook.attach_to_module(feed_forward_module)

                self.layer_hooks.append(layer_hook)
                logger.info(f"Attached hook to feed_forward MLP in layer {layer_idx}")
...
def capture_real_pixtral_mlp_inputs(model_args) -> Dict[int, torch.Tensor]:
    """
    Capture real input tensors to all 24 feed_forward PixtralMLP layers by running the full multimodal model
    ...
    Returns:
        Dict[int, torch.Tensor]: Dictionary mapping layer index to input tensor for each feed_forward MLP layer
    """
    logger.info("Capturing real feed_forward MLP inputs from all 24 layers in full multimodal model...")
    ...
    try:
        with torch.no_grad():
            ...
            try:
                # Just run a forward pass to capture the feed_forward MLP input
...
def run_ttnn_pixtral_mlp(mesh_device, device_name: str, model_args, state_dict, input_tensor, layer_idx=0):
    """Run TTNN PixtralMLP implementation for a specific mesh device and layer."""
    logger.info(f"Running TTNN feed_forward MLP layer {layer_idx} on {device_name} device...")

    # MLP parameters
    dim = model_args.vision_dim  # 1024
    hidden_dim = model_args.vision_hidden_dim  # 4096

    # Use the specific transformer layer's feed_forward prefix
    state_dict_prefix = f"vision_tower.transformer.layers.{layer_idx}.feed_forward."

    logger.info(f"MLP config: dim={dim}, hidden_dim={hidden_dim}, layer={layer_idx}")
    ...
    # MLP expects input in TILE_LAYOUT with appropriate memory config
...
def comprehensive_pixtral_mlp_test(
    mesh_devices: Dict[str, Any],
    pcc_threshold: float = 0.99,
    atol_threshold: float = 1e-2,
    rtol_threshold: float = 1e-2,
    save_results: bool = True,
    results_dir: str = "comprehensive_pixtral_mlp_test_results",
) -> ComprehensivePixtralMLPTestResult:
    """
    ...
    Returns:
        ComprehensivePixtralMLPTestResult with all statistics and metrics
    """

    logger.info("=" * 80)
    logger.info("COMPREHENSIVE MISTRAL24B PIXTRAL MLP TEST WITH MULTIMODAL REAL INPUTS")
    logger.info("=" * 80)
    ...
    test_config = {
        ...
        "layer": "feed_forward PixtralMLP (gate_proj, up_proj, down_proj, SiLU)",
    ...
    logger.info(f"feed_forward MLP params: dim={dim}, hidden_dim={hidden_dim}")
    ...
    captured_inputs_by_layer = capture_real_pixtral_mlp_inputs(model_args)
    ...
    for layer_idx in captured_inputs_by_layer.keys():
        # Get the feed_forward MLP from this specific transformer layer
        feed_forward_ref = model_args.reference_vision_mlp(layer_idx=layer_idx)

        # Load the appropriate weights for this layer
        feed_forward_prefix = f"vision_tower.transformer.layers.{layer_idx}.feed_forward"
        feed_forward_state_dict = {
            k[len(feed_forward_prefix + ".") :]: v
            for k, v in state_dict.items()
            if k.startswith(feed_forward_prefix + ".")
        }

        if feed_forward_state_dict:
            logger.info(
                f"Loading layer {layer_idx} feed_forward weights with keys: {list(feed_forward_state_dict.keys())}"
            )
            feed_forward_ref.load_state_dict(feed_forward_state_dict)
            feed_forward_ref.eval()

            # Run PyTorch reference for this layer
            input_tensor = captured_inputs_by_layer[layer_idx]
            with torch.no_grad():
                torch_output = feed_forward_ref(input_tensor)
    ...
    for layer_idx in captured_inputs_by_layer.keys():
        ...
        for device_name, mesh_device in mesh_devices.items():
            ...
            try:
                tt_output_torch = run_ttnn_pixtral_mlp(
    ...
    result = ComprehensivePixtralMLPTestResult(
    ...
    if save_results:
        ...
        # Save JSON report
        json_path = results_path / "comprehensive_pixtral_mlp_test_results.json"
...
def test_comprehensive_pixtral_mlp_cross_mesh(mesh_device_configs):
    """Comprehensive feed_forward MLP test across multiple mesh device configurations."""
    logger.info("Running comprehensive feed_forward MLP test across mesh devices...")
    ...
    try:
        result = comprehensive_pixtral_mlp_test(mesh_devices=available_devices, pcc_threshold=0.99, save_results=True)

        # Assert overall test passed
        assert result.test_passed, f"feed_forward MLP test failed. Check detailed results in logs."
        ...
        for device_name, mesh_result in result.mesh_results.items():
            ...
            ), f"feed_forward MLP test failed on {device_name} device for representative layer"
...
def test_single_device_pixtral_mlp(mesh_device):
    """Test feed_forward MLP on a single mesh device configuration."""
    logger.info("Running single device feed_forward MLP test...")
    ...
    try:
        result = comprehensive_pixtral_mlp_test(
            mesh_devices={device_name: mesh_device},
            pcc_threshold=0.99,
            save_results=True,
            results_dir=f"pixtral_mlp_test_results_{device_name.lower()}",
        )

        assert result.test_passed, f"feed_forward MLP test failed on {device_name}"

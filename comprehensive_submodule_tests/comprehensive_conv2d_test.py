#!/usr/bin/env python3
"""
Comprehensive Conv2d Test for Mistral24B Model
...
class ComparisonMetrics:
    ...
    pcc: float
    pcc_per_channel: Optional[np.ndarray]
    atol: float
    rtol: float
    mse: float
    mae: float
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "pcc": self.pcc,
            "pcc_per_channel": self.pcc_per_channel.tolist() if self.pcc_per_channel is not None else None,
            "atol": self.atol,
            "rtol": self.rtol,
            "mse": self.mse,
            "mae": self.mae,
...
@dataclass
class ComprehensiveConv2dTestResult:
...
def compute_comparison_metrics(
    ...
    # Per-channel PCC for conv outputs
    pcc_per_channel = None
    if len(torch_tensor.shape) == 4:  # (B, C, H, W)
        pcc_per_channel = []
        for c in range(torch_tensor.shape[1]):
            torch_channel = torch_tensor[:, c, :, :].flatten()
            ttnn_channel = ttnn_tensor[:, c, :, :].flatten()
            if torch_channel.numel() > 1:
                channel_pcc = torch.corrcoef(torch.stack([torch_channel, ttnn_channel]))[0, 1].item()
                pcc_per_channel.append(channel_pcc)
        pcc_per_channel = np.array(pcc_per_channel) if pcc_per_channel else None
    ...
    return ComparisonMetrics(
        pcc=pcc,
        pcc_per_channel=pcc_per_channel,
        atol=atol_achieved,
        rtol=rtol_achieved,
        mse=mse,
        mae=mae,
...
class InputCaptureHook:
    """Hook to capture inputs to the patch_conv layer."""
    ...
    def __call__(self, module, input, output):
        ...
        if len(input) > 0:
            # Capture the first input tensor (the image tensor)
            captured_input = input[0].clone().detach()
            self.captured_inputs.append(captured_input)
            logger.info(f"Captured input tensor with shape: {captured_input.shape}")
    ...
    def get_all_inputs(self):
        ...
        return self.captured_inputs.copy()


def capture_real_conv2d_inputs(model_args) -> torch.Tensor:
    """
    Capture real input tensor to the Conv2d layer by running the full multimodal model
    ...
    Returns:
        Captured input tensor from the Conv2d layer
    """
    logger.info("Capturing real Conv2d input from full multimodal model...")
    ...
    # Create hook to capture inputs to patch_conv
    input_hook = InputCaptureHook()

    # Find the patch_conv module in the vision tower and attach hook
    patch_conv_module = None
    for name, module in reference_model.named_modules():
        if "patch_conv" in name.lower() or (name.endswith("patch_conv")):
            patch_conv_module = module
            logger.info(f"Found patch_conv module at: {name}")
            break
        # Also check for Conv2d layers in vision tower
        elif (
            "vision" in name.lower()
            and hasattr(module, "weight")
            and len(getattr(module, "weight", torch.tensor([])).shape) == 4
        ):
            patch_conv_module = module
            logger.info(f"Found Conv2d module in vision tower at: {name}")
            break

    if patch_conv_module is None:
        raise RuntimeError("Could not find patch_conv module in the multimodal model")

    # Attach hook
    input_hook.attach_to_module(patch_conv_module)
    ...
    try:
        with torch.no_grad():
            ...
            try:
                # Just run a forward pass to capture the conv2d input
...
def run_ttnn_conv2d(mesh_device, device_name: str, model_args, state_dict, input_tensor):
    """Run TTNN Conv2d implementation for a specific mesh device."""
    logger.info(f"Running TTNN Conv2d on {device_name} device...")

    # Conv2d parameters
    in_channels = 3
    out_channels = model_args.vision_dim  # 1024
    kernel_size = model_args.vision_patch_size  # 14
    stride = model_args.vision_patch_size  # 14
    bias = False
    ...
    tt_output = tt_model(input_tensor.float())
    ...
    # Post-process TTNN output to match reference format
    tt_output_torch = tt_output_torch[0, ..., :out_channels]  # Select first device output
    tt_output_torch = tt_output_torch.unsqueeze(0)  # Restore batch dim

    # Reshape to conv output format: (N, C_out, H_out, W_out)
    batch_size = input_tensor.shape[0]
    image_height = input_tensor.shape[2]
    image_width = input_tensor.shape[3]
    H_out = image_height // stride
    W_out = image_width // stride
    tt_output_torch = tt_output_torch.permute(0, 2, 1).reshape(batch_size, out_channels, H_out, W_out)

    logger.info(f"{device_name} output shape: {tt_output_torch.shape}")
    return tt_output_torch


def comprehensive_conv2d_test(
    mesh_devices: Dict[str, Any],
    pcc_threshold: float = 0.99,
    atol_threshold: float = 1e-2,
    rtol_threshold: float = 1e-2,
    save_results: bool = True,
    results_dir: str = "comprehensive_conv2d_test_results",
) -> ComprehensiveConv2dTestResult:
    """
    ...
    Returns:
        ComprehensiveConv2dTestResult with all statistics and metrics
    """

    logger.info("=" * 80)
    logger.info("COMPREHENSIVE MISTRAL24B CONV2D TEST WITH MULTIMODAL REAL INPUTS")
    logger.info("=" * 80)
    ...
    logger.info(
        f"Conv2d params: in_channels={in_channels}, out_channels={out_channels}, "
        f"kernel_size={kernel_size}, stride={stride}, bias={bias}"
    )
    ...
    input_tensor = capture_real_conv2d_inputs(model_args)
    ...
    reference_conv = model_args.reference_conv2d_patch()
    first_layer_prefix = "vision_tower.patch_conv."
    partial_state_dict = {
        k[len(first_layer_prefix) :]: v for k, v in state_dict.items() if k.startswith(first_layer_prefix)
    }
    reference_conv.load_state_dict(partial_state_dict)

    # Run PyTorch reference
    with torch.no_grad():
        torch_output = reference_conv(input_tensor)
    ...
    for device_name, mesh_device in mesh_devices.items():
        ...
        try:
            tt_output_torch = run_ttnn_conv2d(mesh_device, device_name, model_args, state_dict, input_tensor)
            ...
            device_test_passed = (
                ...
                and vs_torch_metrics.atol <= atol_threshold
                and vs_torch_metrics.rtol <= rtol_threshold
            ...
            mesh_results[device_name] = MeshDeviceResult(
                ...
                vs_torch_metrics=ComparisonMetrics(
                    0, None, float("inf"), float("inf"), float("inf"), float("inf"), float("inf"), float("inf"), 0, 0
                ),
                test_passed=False,
            )
    ...
    weight_stats = compute_tensor_stats(weight_tensor, "conv_weight")
    logger.info(
    ...
    result = ComprehensiveConv2dTestResult(
    ...
    if save_results:
        ...
        # Save JSON report
        json_path = results_path / "comprehensive_conv2d_test_results.json"
...
def test_comprehensive_conv2d_cross_mesh(mesh_device_configs):
    """Comprehensive Conv2d test across multiple mesh device configurations."""
    logger.info("Running comprehensive Conv2d test across mesh devices...")
    ...
    try:
        result = comprehensive_conv2d_test(mesh_devices=available_devices, pcc_threshold=0.99, save_results=True)

        # Assert overall test passed
        assert result.test_passed, f"Conv2d test failed. Check detailed results in logs."

        # Assert each device passed
        for device_name, mesh_result in result.mesh_results.items():
            assert mesh_result.test_passed, f"Conv2d test failed on {device_name} device"
...
def test_single_device_conv2d(mesh_device):
    """Test Conv2d on a single mesh device configuration."""
    logger.info("Running single device Conv2d test...")
    ...
    try:
        result = comprehensive_conv2d_test(
            mesh_devices={device_name: mesh_device},
            pcc_threshold=0.99,
            save_results=True,
            results_dir=f"conv2d_test_results_{device_name.lower()}",
        )

        assert result.test_passed, f"Conv2d test failed on {device_name}"

#!/usr/bin/env python3
"""
Comprehensive PixtralTransformer Test for Mistral24B Model
...
Tests the PixtralTransformer layer: PixtralTransformer with 24 layers
"""
...
@dataclass
class ComprehensivePixtralTransformerTestResult:
...
class InputCaptureHook:
    """Hook to capture inputs to the PixtralTransformer layer."""
    ...
    def __call__(self, module, input, output):
        ...
        # PixtralTransformer forward signature: forward(self, x, return_intermediate=None, mask=None, position_embeddings=None)
        if len(input) > 0:
            # Capture the main input tensor
            captured_input = input[0].clone().detach()
            self.captured_inputs.append(captured_input)
            logger.info(f"Captured PixtralTransformer input tensor with shape: {captured_input.shape}")
...
def capture_real_pixtral_transformer_inputs(
    model_args,
) -> Tuple[torch.Tensor, torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
    """
    Capture real input tensors to the PixtralTransformer layer by running the full multimodal model
    ...
    Returns:
        Tuple of (input_tensor, attention_mask, position_embeddings)
    """
    logger.info("Capturing real PixtralTransformer inputs from full multimodal model...")
    ...
    # Create hook to capture inputs to PixtralTransformer
    input_hook = InputCaptureHook()

    # Find the PixtralTransformer module in the vision tower and attach hook
    transformer_module = None
    for name, module in reference_model.named_modules():
        if "transformer" in name.lower() and "vision" in name.lower() and hasattr(module, "forward"):
            # Check if this looks like the transformer (has layers)
            if hasattr(module, "layers") or hasattr(module, "resblocks"):
                transformer_module = module
                logger.info(f"Found PixtralTransformer module at: {name}")
                break

    if transformer_module is None:
        raise RuntimeError("Could not find PixtralTransformer module in the multimodal model")

    # Attach hook
    input_hook.attach_to_module(transformer_module)
    ...
    try:
        with torch.no_grad():
            ...
            try:
                # Just run a forward pass to capture the transformer input
            ...
            batch_size, seq_len, dim = captured_input.shape

            # Create attention mask (all ones for no masking)
    ...
    logger.info("Successfully captured real input tensors from multimodal model")
    return captured_input, attention_mask, position_embeddings


def run_ttnn_pixtral_transformer(
    mesh_device, device_name: str, model_args, state_dict, input_tensor, attention_mask, position_embeddings
):
    """Run TTNN PixtralTransformer implementation for a specific mesh device."""
    logger.info(f"Running TTNN PixtralTransformer on {device_name} device...")

    # Transformer parameters
    n_layers = model_args.vision_n_layers  # 24
    dim = model_args.vision_dim  # 1024

    first_layer_prefix = "vision_tower.transformer."
    dtype = ttnn.bfloat16

    # Create TTNN PixtralTransformer model
    ...
    tt_output = tt_model(attention_input, mask=tt_mask, position_embeddings=(cos_t, sin_t))
...
def comprehensive_pixtral_transformer_test(
    mesh_devices: Dict[str, Any],
    pcc_threshold: float = 0.99,
    atol_threshold: float = 1e-2,
    rtol_threshold: float = 1e-2,
    save_results: bool = True,
    results_dir: str = "comprehensive_pixtral_transformer_test_results",
) -> ComprehensivePixtralTransformerTestResult:
    """
    ...
    Returns:
        ComprehensivePixtralTransformerTestResult with all statistics and metrics
    """

    logger.info("=" * 80)
    logger.info("COMPREHENSIVE MISTRAL24B PIXTRAL TRANSFORMER TEST WITH MULTIMODAL REAL INPUTS")
    logger.info("=" * 80)
    ...
    test_config = {
        ...
        "layer": "PixtralTransformer (24 layers)",
    ...
    n_layers = model_args.vision_n_layers  # 24
    dim = model_args.vision_dim  # 1024
    heads = model_args.vision_attn_n_heads  # 16

    logger.info(f"PixtralTransformer params: n_layers={n_layers}, dim={dim}, heads={heads}")

    # ==================== CAPTURE REAL INPUTS FROM MULTIMODAL MODEL ====================
    logger.info("\n" + "-" * 50)
    logger.info("CAPTURING REAL INPUTS FROM FULL MULTIMODAL MODEL")
    logger.info("-" * 50)

    # Capture real input tensors using hooks
    input_tensor, attention_mask, position_embeddings = capture_real_pixtral_transformer_inputs(model_args)
    ...
    logger.info(f"Position embeddings shape: cos={position_embeddings[0].shape}, sin={position_embeddings[1].shape}")
    ...
    with torch.no_grad():
        torch_output = reference_model(
            input_tensor, attention_mask=attention_mask, position_embeddings=position_embeddings
        )[0]
    ...
    for device_name, mesh_device in mesh_devices.items():
        ...
        try:
            tt_output_torch = run_ttnn_pixtral_transformer(
                mesh_device, device_name, model_args, state_dict, input_tensor, attention_mask, position_embeddings
            )
    ...
    logger.info(f"Attention mask stats: shape={attention_mask_stats.shape}")
    logger.info(f"Position embeddings stats: cos_shape={cos_stats.shape}, sin_shape={sin_stats.shape}")
    ...
    result = ComprehensivePixtralTransformerTestResult(
    ...
    if save_results:
        ...
        # Save JSON report
        json_path = results_path / "comprehensive_pixtral_transformer_test_results.json"
...
def test_comprehensive_pixtral_transformer_cross_mesh(mesh_device_configs):
    """Comprehensive PixtralTransformer test across multiple mesh device configurations."""
    logger.info("Running comprehensive PixtralTransformer test across mesh devices...")
    ...
    try:
        result = comprehensive_pixtral_transformer_test(
            mesh_devices=available_devices, pcc_threshold=0.99, save_results=True
        )

        # Assert overall test passed
        assert result.test_passed, f"PixtralTransformer test failed. Check detailed results in logs."

        # Assert each device passed
        for device_name, mesh_result in result.mesh_results.items():
            assert mesh_result.test_passed, f"PixtralTransformer test failed on {device_name} device"
...
def test_single_device_pixtral_transformer(mesh_device):
    """Test PixtralTransformer on a single mesh device configuration."""
    logger.info("Running single device PixtralTransformer test...")
    ...
    try:
        result = comprehensive_pixtral_transformer_test(
            ...
            pcc_threshold=0.99,
            save_results=True,
            results_dir=f"pixtral_transformer_test_results_{device_name.lower()}",
        )

        assert result.test_passed, f"PixtralTransformer test failed on {device_name}"

...
def test_mlp_inference(batch, num_chunks, mesh_device, use_program_cache, reset_seeds):
    dtype = ttnn.bfloat16
    model_args = ModelArgs(mesh_device)
    state_dict = model_args.load_state_dict()
    # print(state_dict[0].dtype())

    # Ref model needs partial state dict, but our models use full state dict keys as cached weight names
    first_layer_prefix = "visual.blocks.0.mlp."
    partial_state_dict = {
    ...
    torch_input = torch.load("/home/vision_mlp_down_proj.pt").float()
    print(torch_input.dtype)

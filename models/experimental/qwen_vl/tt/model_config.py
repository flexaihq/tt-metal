...
class ModelArgs:
    ...
    def load_state_dict(self):
        if self.dummy_weights:
            ...
            if self.from_hf_url:
                ...
                if "Qwen2.5-VL-7B" in self.model_name:
                    model = AutoModelForImageTextToText.from_pretrained(self.CKPT_DIR, torch_dtype=torch.bfloat16)
                else:
        ...
        if self.checkpoint_type == CheckpointType.HuggingFace:
            if "gemma-3-4b" in self.model_name or "Qwen2.5-VL-7B" in self.model_name:
    ...
    def reference_vision_transformer(self, wrap=True, load_checkpoint=False):
        if self.checkpoint_type == CheckpointType.HuggingFace:
            ...
            if self.dummy_weights and not load_checkpoint:
                ...
                from transformers import Gemma3ForConditionalGeneration, Qwen2_5_VLForConditionalGeneration
    ...
    def reference_vision_attention(self):
        model = self.reference_vision_transformer(wrap=False)
        if "gemma-3-4b" in self.model_name:
            layer = model.vision_tower.vision_model.encoder.layers[0].self_attn
        elif "Qwen2.5-VL-7B" in self.model_name:
    ...
    def reference_embedding(self, reference_model=None):
        if self.checkpoint_type == CheckpointType.Meta:
            ...
            if reference_model is None:
                ...
                # layer = reference_model.embed_tokens
...
class HfAttentionWrapper:
    ...
    def forward(self, x, start_pos, freqs_cis_i, mask=None):
        ...
        # position_ids = torch.full((batch_size, 1), value, dtype=torch.long)
...
class HfModelWrapper:
    ...
    def forward(self, inputs_embeds, start_pos, mode="decode"):
        ...
        # )
        position_ids = torch.full((3, 1, 1), start_pos, dtype=torch.long)

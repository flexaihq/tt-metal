...
def process_real_vision_inputs(messages, model_args):
    ...
    print("input_ids shape:", input_ids.shape)  # Should be (batch_size, seq_len)
    pixel_values = None
...
def run_generation_exactly_like_test_end2end(
    ...
    # print("vision output before squeeze:", vision_output.shape)
    # comp_vision_output = ttnn.to_torch(ttnn.from_device(vision_output)).squeeze(0)
    ...
    # inputs_embeds = host_embedding(input_ids)

    # print("input_ids:", input_ids.shape)
    # image_features = comp_vision_output
    # special_image_mask = (input_ids == model_args.image_token_index).unsqueeze(-1)
    # special_image_mask = special_image_mask.expand_as(inputs_embeds).to(inputs_embeds.device)
    # image_features = image_features.to(inputs_embeds.device, inputs_embeds.dtype)
    # inputs_embeds = inputs_embeds.masked_scatter(special_image_mask, image_features)

    # print("input_embeddings:", inputs_embeds.shape)
    ...
    # # Preprocessing (exactly like test_end2end.py)
    ...
    print("input_tokens_prefill_pt shape:", input_tokens_prefill_pt.shape)
    # input_tokens_prefill_pt
    # Prefill (exactly like test_end2end.py)

...
class Generator:
    ...
    def generate(
        ...
        if "gemma-3" in self.model_args[0].model_name:  # Gemma-3 is a single-user model
            vision_images = model_input.pixel_values
            vision_mask = model_input.attention_mask
    ...
    def text_completion(
        ...
        if "gemma-3" in self.model_args[0].model_name:  # TODO: Handle using encode_prompt_hf
            model_input = self.formatter.apply_chat_template(
                [content], add_generation_prompt=True, tokenize=True, return_dict=True, return_tensors="pt"
        ...
        if "gemma-3" in self.model_args[0].model_name:
            self.tokenizer = self.formatter

...
class TtQwen2_5_VLVisionSdpaAttention(LightweightModule):
    ...
    def forward(self, hidden_states, cu_seqlens, position_embeddings):
        ...
        attention_mask = torch.zeros([1, seq_len, seq_len], dtype=torch.bool, device=cu_seqlens.device)
        ...
        tt_attention_mask = ttnn.from_torch(
            attention_mask,
            device=self.mesh_device,  # or your target mesh device
            dtype=ttnn.bfloat16,  # or keep bool if TT supports it, but usually you'd cast
            layout=ttnn.TILE_LAYOUT,
            memory_config=ttnn.DRAM_MEMORY_CONFIG,
        )
